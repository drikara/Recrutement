...
export async function GET(request: NextRequest) {
  try {
    ...
    function getSessionAgenceType(candidate: any): string {
      return candidate.session?.agenceType || ''
    }
    ...
    candidates.forEach((candidate: any, index: number) => {
      ...
      const agenceType = getSessionAgenceType(candidate)
    ...
    const sessionCreators = Array.from(
      new Set(candidates.map(c => c.session?.createdBy?.name || 'Non renseigné'))
    )
